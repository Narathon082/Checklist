...
// Question dictionaries fetched dynamically from the database
$category_lookup = [];
$catRes = $conn->query("SELECT * FROM `form_categories`");
if ($catRes) {
    while ($r = $catRes->fetch_assoc()) {
        $category_lookup[strtolower($r['code'])] = $r;
    }
}

$questions_dim = [];
$fieldsRes = $conn->query("SELECT * FROM `form_fields` WHERE `step` = 2 AND `status` = 'active' ORDER BY `sort_order` ASC, `id` ASC");
if ($fieldsRes) {
    while ($r = $fieldsRes->fetch_assoc()) {
        $cat = strtolower($r['category']);
        if (!isset($questions_dim[$cat])) {
            $cat_data = $category_lookup[$cat] ?? null;
            $title = $cat_data ? $cat_data['title'] : strtoupper($cat);
        ...
        $questions_dim[$cat]['items'][$r['field_code']] = $r['label'] . (!empty($r['description']) ? ' (' . $r['description'] . ')' : '');
    }
}

$questions_page5 = [];
$fieldsRes4 = $conn->query("SELECT * FROM `form_fields` WHERE `step` = 4 AND `status` = 'active' ORDER BY `sort_order` ASC, `id` ASC");
if ($fieldsRes4) {
    while ($r = $fieldsRes4->fetch_assoc()) {
        $cat = strtolower($r['category']);
        if (!isset($questions_page5[$cat])) {
            $cat_data = $category_lookup[$cat] ?? null;
            $title = $cat_data ? $cat_data['title'] : strtoupper($cat);
        ...
        $questions_page5[$cat]['labels'][$r['field_code']] = $r['label'] . (!empty($r['description']) ? ' (' . $r['description'] . ')' : '');
        $questions_page5[$cat]['count']++;
    }
}
?>
<!DOCTYPE html>
